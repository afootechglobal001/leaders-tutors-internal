...
try {
    ...
    ////////////////// Validation //////////////////
    validateEmptyField($yearId, 'YEAR ID');
    ...
    $existingYear = selectQuery(
        $conn,
        "SELECT yearId FROM YEARS_TAB WHERE departmentId = ? AND yearValue = ? AND yearId != ?",
        "iis",
        [$departmentId, $yearValue, $yearId]
    );
    ...
    ////////////////// Update year //////////////////
    $updateQuery = "UPDATE YEARS_TAB SET yearValue = ?, statusId = ?, updatedBy = ? WHERE yearId = ?";
    $updateParams = [$yearValue, $statusId, $loginStaffId, $yearId];
    $updateTypes = "siss";
    updateQuery($conn, $updateQuery, $updateTypes, $updateParams);

    //// delete existing exams and subjects for the year
    $deleteExamsQuery = "DELETE FROM YEAR_EXAMS_TAB WHERE yearId = ?";
    deleteQuery($conn, $deleteExamsQuery, 's', [$yearId]);
    $deleteSubjectsQuery = "DELETE FROM YEAR_SUBJECTS_TAB WHERE yearId = ?";
    deleteQuery($conn, $deleteSubjectsQuery, 's', [$yearId]);
